...
class Wrapper(games.Sprite):
    """Sprite, którego tor lotu 'owija się wokół ekranu"""
    def update(self):
        """Przenieś sprite'a na przeciwległy koniec ekranu"""
...
class Missile(Collider):
    """Pocisk wystrzelony przez gracza"""
    ...
    def __init__(self, ship_x, ship_y, ship_angle):
        """Inicjalizacja sprite'a pocisku"""
        Missile.sound.play()
        angle = ship_angle * math.pi / 180

        #obliczanie pozycji początkowej pocisku
...
class Game(object):
    """Sama gra"""
    def __init__(self):
        """Inicjalizacja obiektu klasy Game"""
        # ustaw poziom 0
        self.level = 0
    ...
    def advance(self):
        """Przejdź do kolejnego poziomu gry"""
        self.level += 1

        # wielkość obszaru ochronnego wokól statku
        BUFFER = 150

        # utwórz nowe asteroidy
        for i in range(self.level):
            # oblicz współrzędne x i y zapewniające minimum odległości od statku
            # określ minimalną odległość wzdłuż osi x oraz wdłuż osi y
            x_min = random.randrange(BUFFER)
            y_min = BUFFER - x_min

            # wyznacz odległość wzdłuż osi x oraz wdłuż osi y
            ...
            #odwtórz dźwięk przejścia do nowego poziomu (nie dot. 1-szego poziomu)
            if self.level > 1:
                self.sound.play()
    def end(self):
        """Zakończ grę"""
        # Pokazuj komunikat o końcu gry przez 5 sekund
